...
Para esta etapa se ha definido el job en el archivo link:jobs/maven/compile-simple.yml[]. Este job se ejecuta incondicionalmente en todas las ramas al hacer push, excepto en ramas auxiliares si el flag COMPILE_AUX_BRANCHES está a false. Nótese que en la rama hotfix (si existe) también se ejecuta este job. En este ejemplo, el job es bastante simple:
...
Para esta etapa se ha definido el job en el archivo link:jobs/maven/sonar-simple.yml[]. Este job se ejecuta incondicionalmente en todas las ramas al hacer push, excepto en ramas auxiliares si el flag SONAR_AUX_BRANCHES está a false. Nótese que en la rama hotfix (si existe) también se ejecuta este job. Se ejecuta Sonarqube sobre el código fuente para localizar bugs, code-smells y vulnerabilidades SAST.
...
Para esta etapa se ha definido el job en el archivo link:jobs/maven/register-simple.yml[]. Este job se ejecuta en master, hotfix y develop (si el flag REGISTER_DEVELOP es true) al hacer push. No se ejecuta en ramas auxiliares.
